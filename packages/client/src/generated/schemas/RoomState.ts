...
//
// GENERATED USING @colyseus/schema 4.0.21
//
...
export class RoomState extends Schema {
  ...
  @type({ map: EntitySchema }) public humans: MapSchema<EntitySchema> =
    new MapSchema<EntitySchema>();
  @type({ map: EntitySchema }) public agents: MapSchema<EntitySchema> =
    new MapSchema<EntitySchema>();
  @type({ map: EntitySchema }) public objects: MapSchema<EntitySchema> =
    new MapSchema<EntitySchema>();
  @type({ map: ZoneSchema }) public zones: MapSchema<ZoneSchema> = new MapSchema<ZoneSchema>();
  @type({ map: OrganizationSchema }) public organizations: MapSchema<OrganizationSchema> =
    new MapSchema<OrganizationSchema>();
  ...
  @type({ map: NoticeSchema }) public notices: MapSchema<NoticeSchema> =
    new MapSchema<NoticeSchema>();
  @type({ map: FacilitySchema }) public facilities: MapSchema<FacilitySchema> =
    new MapSchema<FacilitySchema>();
  @type({ map: MeetingReservationSchema })
  public reservations: MapSchema<MeetingReservationSchema> =
    new MapSchema<MeetingReservationSchema>();
  @type({ map: KanbanBoardSchema }) public boards: MapSchema<KanbanBoardSchema> =
    new MapSchema<KanbanBoardSchema>();
  @type({ map: WhiteboardSchema }) public whiteboards: MapSchema<WhiteboardSchema> =
