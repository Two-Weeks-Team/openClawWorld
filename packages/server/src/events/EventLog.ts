...
export class EventLog {
  ...
  getSince(
    ...
    if (index === -1) {
      // Cursor expired or invalid — skip past events, start from current position
    ...
    const nextCursor = events.length > 0 ? events[events.length - 1].cursor : this.getCurrentCursor();
